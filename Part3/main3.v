...
// Parser Constructor
fn (mut self Parser) initparser(lexer Lexer, emitter Emitter) {
	self.lexer = lexer
...
//########################### EMITTER ###################################
//#######################################################################
fn (mut self Emitter) initemitter(fullpath string) {
	self.fullpath = fullpath
	self.header = ""
	self.code = ""
}
fn (mut self Emitter) emit(code string) {
	self.code += code
}
fn (mut self Emitter) emitline(code string) {
	self.code += code + `\n`.str()
}
fn (mut self Emitter) headerline(code string) {
	self.header += code + `\n`.str()
}
fn (mut self Emitter) writefile() {
	mut outputfile := os.open_file(self.fullpath,'w+') or { panic(err) }
	outputfile.writeln(self.header + self.code)
}
//#######################################################################
//########################### MAIN() ####################################
//#######################################################################
fn main() {
	...
	mut lexer := &Lexer{}
	mut parser := &Parser{}
	mut emitter := &Emitter{"out1.c", '',''}

	lexer.initlexer(input)
	parser.initparser(lexer, emitter)

    parser.program() 				//# Start the parser.
	parser.emitter.writefile() 		//# Write the output to file.
